            ...
            <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto bg-white border-2 border-slate-200">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2"><Label className="text-slate-700 font-semibold">{language === 'fr' ? "Date Exp. Assurance" : "تاريخ انتهاء التأمين"}</Label><Input type="date" value={formData.insurance_expiry ? formData.insurance_expiry.split('T')[0] : ''} onChange={(e) => setFormData({...formData, insurance_expiry: e.target.value})} onFocus={() => playSyntheticSound('click')} className="bg-white border-2 border-slate-300 focus:border-cyan-500" /></div>
                  <div className="space-y-2"><Label className="text-slate-700 font-semibold">{language === 'fr' ? "Date Exp. Contrôle Technique" : "تاريخ انتهاء الفحص الفني"}</Label><Input type="date" value={formData.technical_control_expiry ? formData.technical_control_expiry.split('T')[0] : ''} onChange={(e) => setFormData({...formData, technical_control_expiry: e.target.value})} onFocus={() => playSyntheticSound('click')} className="bg-white border-2 border-slate-300 focus:border-cyan-500" /></div>
                  <div className="space-y-2"><Label className="text-slate-700 font-semibold">{language === 'fr' ? "IMEI Traceur GPS" : "IMEI جهاز تتبع GPS"}</Label><Input value={formData.gps_imei} onChange={(e) => setFormData({...formData, gps_imei: e.target.value})} onFocus={() => playSyntheticSound('click')} placeholder="353701097887430" className="bg-white border-2 border-slate-300 focus:border-cyan-500" /></div>
